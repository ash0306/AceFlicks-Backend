...
namespace MovieBookingBackend.Repositories
{
    public class TheatreRepository : IRepository<int, Theatre>
    {
        ...
        {
            var theatres = await _context.Theatres.Include(t => t.Showtimes).ThenInclude(s => s.Movie).ToListAsync();
        ...
        {
            var theatre = await _context.Theatres.Include(t => t.Showtimes).ThenInclude(s => s.Movie).FirstOrDefaultAsync(t => t.Id == key);
            if (theatre == null)
            {
                throw new NoSuchTheatreException($"No theatre with ID {key} was found");
            }
